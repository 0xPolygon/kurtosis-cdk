...
# cp -r /tmp/tmp.XKAVPPJ6VF/datafile/datastream.* templates/contract-deploy/erigon-data/

# chmod a+rw templates/contract-deploy/erigon-data
# chown -R 1000:1000 templates/contract-deploy/erigon-data


# kurtosis assumes the name if the network is "kurtosis" so we need to do some renameing
# find . -type f -name 'dynamic-migrationexample*' | while read conf_file; do new_file=$(echo $conf_file | sed 's/migrationexample/kurtosis/'); mv $conf_file $new_file; done
# sed -i 's/migrationexample/kurtosis/' ./templates/contract-deploy/dynamic-kurtosis-chainspec.json

# try to spin up the new network
kurtosis run --enclave cdk2 --args-file sepolia-erigon-test.yml .
#+end_src

Now that the network is up and running, most likely the sequencer is
going to be a bit messed up...

* Questions

- There is no guidance around the creation of the erigon specific
  genesis files. Are we going to do that for the partners or should we
  document that as well?
- The instructions for syncing erigon seem like they would need to be
  done after the network is halted? If we generate the datastream file
  ahead of time and sync erigon when the legacy sequencer is still
  running, won't the erigon data be missing batches?
- Since the legacy sequence supports generating a data file and also
  running the data streamer service, is there a reason we wouldn't
  want to use that file or directly connect Erigon to the data stream
- It would be helpful to know which logs to look for in certain
  circumstances. E.g. when the instructions say "Wait for it to sync
  to the tip." it's not obvious what that means. Erigon is showing
  ~Waiting for at least one new block in datastream
  datastreamBlock=3049 last processed block=3049~, does that mean it's
  at the tip? And since the file is offline, what does it even mean to
  reach the tip since the file is behind.
- Now that I've reached the end, I see that the instructions say to
  sync again, so I guess that would allow us to reach the tip properly.
- The exact details of how we should add a new rollup type and do the
  rollup upgrade aren't obvious. I guess we'll need to document that
