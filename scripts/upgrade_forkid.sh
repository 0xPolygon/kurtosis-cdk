...
if [ $TARGET_FORKID -eq 11 ]; then
    ...
    UPGRADED_PARAMS_FILE=.github/tests/combinations/fork11-new-cdk-stack-rollup.yml
elif [ $TARGET_FORKID -eq 12 ]; then
    TAG_TARGET_FORKID=v8.0.0-rc.4-fork.12
    UPGRADED_PARAMS_FILE=.github/tests/combinations/fork12-new-cdk-stack-rollup.yml
elif [ $TARGET_FORKID -eq 13 ]; then
    TAG_TARGET_FORKID=v8.1.0-rc.2-fork.13
    UPGRADED_PARAMS_FILE=.github/tests/combinations/fork13-new-cdk-stack-rollup.yml
fi

if [ $SOURCE_FORKID -eq 9 ]; then
    PARAMS_FILE=.github/tests/combinations/fork9-new-cdk-stack-rollup.yml
elif [ $SOURCE_FORKID -eq 11 ]; then
    PARAMS_FILE=.github/tests/combinations/fork11-new-cdk-stack-rollup.yml
elif [ $SOURCE_FORKID -eq 12 ]; then
    PARAMS_FILE=.github/tests/combinations/fork12-new-cdk-stack-rollup.yml
fi
