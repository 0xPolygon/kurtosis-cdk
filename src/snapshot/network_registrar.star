...
def _register_op_geth_network(plan, network_args, network_config, contract_setup_addresses, deployment_stages, op_stack_args):
    ...
    # Generate config artifacts for OP-Geth
    consensus_type = network_args.get("consensus_contract_type")
    aggkit_config = ""
    bridge_config = ""
    genesis_artifact = ""

    # Generate AggKit config for pessimistic, fep, and ecdsa-multisig
    if consensus_type in [
        constants.CONSENSUS_TYPE.pessimistic,
        constants.CONSENSUS_TYPE.ecdsa_multisig,
        "fep",  # fep is not in constants
    ]:
        aggkit_config = _generate_aggkit_config(plan, network_args, contract_setup_addresses, network_sovereign_contract_setup_addresses)

    # Generate bridge config for pessimistic and ecdsa-multisig
    if consensus_type in [
        constants.CONSENSUS_TYPE.pessimistic,
        constants.CONSENSUS_TYPE.ecdsa_multisig,
    ]:
        l2_rpc_url = "http://{}{}:{}".format(
            network_args.get("l2_rpc_name"),
            network_args.get("deployment_suffix"),
            ports_package.HTTP_RPC_PORT_NUMBER,
        )
        bridge_config = _generate_bridge_config(plan, network_args, contract_setup_addresses, network_sovereign_contract_setup_addresses, l2_rpc_url)

    # Store genesis artifact from contracts service
