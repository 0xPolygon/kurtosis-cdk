...
def run(plan, args):
    ...
    deploy_parameters_template = read_file(
        src = "./templates/deploy_parameters.json"
    )
    ...
    bridge_config_template = read_file(
        src = "./templates/bridge-config.toml"
    )
    ...
    plan.exec(
        service_name = "contracts"+args["deployment_idx"],
        recipe = ExecRecipe(command = ["git", "clone", "--depth", "1", "-b", args["zkevm_contracts_branch"], args["zkevm_contracts_repo"], "/opt/zkevm-contracts"])
    )
    plan.exec(
        service_name = "contracts"+args["deployment_idx"],
        recipe = ExecRecipe(command = ["chmod", "a+x", "/opt/contract-deploy/run-contract-setup.sh"])
    )
    ...
    plan.add_service(
        ...
        config = ServiceConfig(
            ...
            ports = {
                "hash-db-server": PortSpec(args["zkevm_hash_db_port"], application_protocol="grpc"),
                "executor-server": PortSpec(args["zkevm_executor_port"], application_protocol="grpc"),
            },
            files = {
                "/etc/": zkevm_configs,
            },
    ...
    plan.add_service(
        ...
        config = ServiceConfig(
            ...
            ports = {
                "trusted-rpc": PortSpec(args["zkevm_rpc_http_port"], application_protocol="http"),
                "data-streamer": PortSpec(args["zkevm_data_streamer_port"], application_protocol="datastream"),
                "pprof": PortSpec(args["zkevm_pprof_port"], application_protocol="http"),
                "prometheus": PortSpec(args["zkevm_prometheus_port"], application_protocol="http"),
            },
            entrypoint = [
    ...
    plan.add_service(
        ...
        config = ServiceConfig(
            image = args["zkevm_node_image"],
            ports = {
                "trusted-aggregator": PortSpec(args["zkevm_aggregator_port"], application_protocol="grpc"),
                "pprof": PortSpec(args["zkevm_pprof_port"], application_protocol="http"),
                "prometheus": PortSpec(args["zkevm_prometheus_port"], application_protocol="http"),
            },
            files = {
    ...
    plan.add_service(
        ...
        config = ServiceConfig(
            ...
            ports = {
                "trusted-rpc": PortSpec(args["zkevm_rpc_http_port"], application_protocol="http"),
                "trusted-ws": PortSpec(args["zkevm_rpc_ws_port"], application_protocol="ws"),
                "pprof": PortSpec(args["zkevm_pprof_port"], application_protocol="http"),
                "prometheus": PortSpec(args["zkevm_prometheus_port"], application_protocol="http"),
            },
            files = {
    ...
    plan.add_service(
        ...
        config = ServiceConfig(
            ...
            ports = {
                "bridge-rpc": PortSpec(args["zkevm_bridge_rpc_port"], application_protocol="http"),
                "bridge-grpc": PortSpec(args["zkevm_bridge_grpc_port"], application_protocol="grpc"),
            },
            files = {
                "/etc/": zkevm_configs,
            },
            ...
            cmd = [
                "run",
                "--cfg", "/etc/zkevm/bridge-config.toml"
            ],
        ),
    )
...
def add_databases(plan, args):
    prover_db_init_artifact = plan.upload_files(
        src = "./templates/prover-db-init.sql",
        name = "prover-db-init.sql"
    )
