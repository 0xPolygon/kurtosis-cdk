...
# Generate runtime init script that runs on every docker-compose up
# Usage: create_init_script.sh <output_init.sh>

OUTPUT_FILE="$1"

cat > "$OUTPUT_FILE" <<'INIT_SCRIPT_EOF'
#!/bin/bash
set -euo pipefail
...
chmod +x "$OUTPUT_FILE"
echo "Init script created: $OUTPUT_FILE"
