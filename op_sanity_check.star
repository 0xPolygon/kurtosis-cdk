...
def check_first_chain_name(args, op_args):
    ...
    chain0 = chains[0]
    network_params0 = chain0.get("network_params")
    op_chain_name = network_params0.get("name")

    deployment_suffix_without_prefix = args.get("deployment_suffix")[1:]
    if op_chain_name != deployment_suffix_without_prefix:
        fail(
            "The name of the first OP chain ({}) does not match the deployment suffix without the leading '-' ({})".format(
                op_chain_name, deployment_suffix_without_prefix
            )
        )
