...
def create_cdk_sequence_sender_config(
    ...
    sequence_sender_config_template = read_file(
        src="../templates/trusted-node/cdk-sequence-sender-config.toml"
    )
    sequence_sender_config_artifact = plan.render_templates(
        name="cdk-sequence-sender-config-artifact",
        config={
            "config.toml": struct(
                data=args
                | {
    ...
    sequence_sender_service_config = ServiceConfig(
        ...
        cmd=[
            "/bin/sh",
            "-c",
            "/app/cdk run --components sequence-sender --custom-network-file /etc/zkevm/genesis.json --cfg /etc/zkevm/config.toml",
        ],
    )
