...
def args_sanity_check(plan, deployment_stages, args, user_args, op_stack_args):
    ...
    if str(args["zkevm_rollup_chain_id"]) != str(op_stack_args["optimism_package"]["chains"][0]["network_params"]["network_id"]) and deployment_stages.get("deploy_op_stack", False):
        fail(
